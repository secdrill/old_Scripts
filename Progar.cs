...
class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        for (int i = 1; i < args.Length; i++)
        {
            ...
            {
                int threads; // Declare variable before use
                if (int.TryParse(args[i + 1], out threads)) // Use pre-declared variable
                {
                    maxThreads = threads;
                    i++;
                }
            ...
            {
                int depth; // Declare variable before use
                if (int.TryParse(args[i + 1], out depth)) // Use pre-declared variable
                {
                    maxDepth = depth;
                    i++;
                }
    ...
    static void GetExcludedFoldersByTier(string basePath, int currentDepth)
    {
        ...
        {
            stopwatch.Stop();
            Console.WriteLine("Scan completed up to depth " + maxDepth + ". Total time: " + stopwatch.Elapsed.TotalSeconds.ToString("F2") + " seconds.");
        }
    }
    ...
    static void ScanDirectory(string currentPath)
    {
        try
        {
            ...
            if (currentCount % 500 == 0)
            {
                ...
                Console.WriteLine("Processed " + currentCount + " directories. Time elapsed: " + elapsed.TotalSeconds.ToString("F2") + " seconds.");
            }
